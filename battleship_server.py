    ...
    if conn in active_matches:
        ...
        try:
            GameMessage.send_msg(
                opp_conn,
                GameMessage(GameMessage.ERROR, {"msg": "Opponent disconnected."})
            )
        except Exception:
            pass
...
def handle_player_data(conn, mask):
    ...
    if msg.msg_type == GameMessage.PLACE_SHIPS:
        ...
        print(f"[READY] Player {p_id} placed ships.")

        if len(match.boards) == 2:
            match.started = True
            print("[GAME START] Both players are ready.")
...
print(f"Battleship Server active on port {PORT}...")
